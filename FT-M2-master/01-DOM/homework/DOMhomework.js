...
let elemento = document.querySelector("#createdBy"); 
elemento.innerHTML += ' ignacio nicoletti'
...
if(todo.complete===true){toDoText.className = 'completeText';}
  ...
  toDoShell.addEventListener('Click',)
...
function buildToDos(toDos) {
  ...
  let arr= toDos.map(function(todo,index){// devuelve un array de divs 
...
// return toDos.map(buildToDo);  --->otra forma 

 // return toDos.map((todo,index)=> buildToDo(todo,index)); -->otra forma
}
...
function addToDo() {
  ...
  if(input.value!==''){               //si hay algo escrito en el input...
